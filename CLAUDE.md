...
Tasteful Intent is a greenfield Tauri desktop Markdown editor for human-authored intentions. The selected `libraryRoot` remains canonical Intent source data. AI always opens a user-selected folder and has two independent source modes: Browse keeps ordered, closeable canonical folder tabs and one root-local session per tab, while Pinned keeps multiple labeled canonical folders and one root-local session per pin. Both modes edit content through the shared Edit, View, and Split modes and the existing atomic-save/mtime-conflict boundary; AI file/folder creation, rename, move, and Trash remain unavailable. Do not add migration adapters for the former PromptPad product. LLM runtime, tags, toolbar, and wiki features are outside v0.2.
...
- Human and AI share a fixed-height 78px two-row Source Card below the space switcher. Human keeps its existing workspace/root anatomy. AI puts the compact `Browse | Pinned` selector at the leading edge of row one. Browse offers Open Folder on row one and ordered closeable folder tabs on row two; opening an exact existing root activates it instead of duplicating it. Pinned shows ordered user labels of one or two Unicode graphemes plus Pin Folder and the active canonical root. Labels default from the folder basename, may duplicate, and can be edited without changing root/session identity. Pinned roots may be anywhere, must be visible non-symlink directories, and may not overlap. Closing a Browse tab removes only that root-local session; Unpin removes only the pin and its root-local session after confirmation when tabs exist. Disk files remain untouched. Every mode/root transition and root close saves all open buffers before changing settings. The two-pane navigation fallback retains the AI controls; content-only does not.
