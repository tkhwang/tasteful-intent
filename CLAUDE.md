...
- The content pane has one top row: an icon-only pane control, scrollable per-space tabs, a Human/AI current-document `RefreshCw`, transient Human save status, and a far-right Human-only mode control; no second header. Reload saves a dirty active document first, then reads only that `{ root, path }` from disk. Save/read failure keeps the current buffer and tab.
