...
- AI space에서만 rendered body 첫 줄 위에 `--muted`·`--fixed-font`·`--type-sm`의 document-path 라인을 표시한다. 표시는 `…/parent/file.md` 끝 두 segment, tooltip은 canonical 전체 경로이며 content와 함께 scroll된다. 경로 텍스트 뒤에 파일 이름 복사(`FileText` + 파일명 라벨)와 파일 이름 포함 canonical 전체 경로 복사(`FolderTree` + 전체 경로 라벨)용 pill 버튼 두 개가 이어지며, 복사 성공 시 해당 아이콘이 잠시 체크 표시로 바뀌고 버튼이 `--space-accent`로 강조된다. 같은 성공 상태는 현재 언어의 polite live-region 메시지로 보조 기술에도 전달한다. Human space와 PDF export에는 표시하지 않고 별도 header를 만들지 않는다.
