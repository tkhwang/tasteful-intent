...
**Goal:** AI 문서 viewer는 항상 사용자가 선택한 폴더를 탐색 root로 열고 그 안의 Markdown 문서를 본다. AI 전체에는 `일반 | 고정` 두 전역 mode가 있으며, 일반은 마지막 폴더 하나를, 고정은 사용자 label을 가진 여러 pinned folder를 관리한다.

**Why:** 파일 picker와 폴더 picker를 섞으면 AI 문서의 source boundary가 불명확해진다. AI viewer의 원칙을 folder-first로 통일하고, 두 mode의 차이를 입력 종류가 아니라 **단일 교체 root**와 **여러 고정 root**의 lifecycle로 제한한다.
...
3. **일반 mode.** 현재 폴더 하나만 유지하고 folder shortcut을 표시하지 않는다. 새 폴더를 열면 이전 일반 root/session을 교체한다.
4. **일반 session 복원.** 마지막 일반 root와 열린 tab·active tab을 settings에 저장하고 재시작 후 복원한다.
...
- `docsRoot: string | null`: 일반 mode에서 사용자가 선택한 canonical root 하나다.
- `tabSessions.docs: { paths: string[]; activePath: string | null }`: 일반 root 기준 canonical relative Markdown path session이다.
...
- 일반: `[Browse selector] | Open Folder action`과 현재 canonical root row. shortcut row에 label group이 없다.
- 고정: `[Pinned selector] | label | label | … | Pin Folder action`과 활성 `[label] canonical root` row.
- mode/root 전환과 root 교체는 dirty-document save barrier 성공 후에만 settings와 mounted workspace를 변경한다.
...
- **일반 root 교체:** dirty 저장 → 새 root canonicalize/scan → 일반 root/session 교체. 여러 일반 root history는 유지하지 않는다.
