...
- Human 파일과 폴더를 생성·이름 변경·이동하고 시스템 휴지통으로 삭제할 수 있다. AI는 Open File로 연 문서 본문만 편집할 수 있으며 구조 변경은 허용하지 않는다.
...
- 클린 설치에서는 Human·AI 모두 기본 root를 정하지 않으며, 3단계 onboarding에서 언어, 테마와 Human/AI 색상, Human 폴더를 정한다. AI는 Open File로 시작한다.
...
- AI용 content-editable `{ root, path }` open-document session과 active source projection `docsRoot` (내부 `docs` key 유지)
...
- workspace 전체 파일명·본문 검색, index, tags, pin
...
클린 settings는 `libraryRoot: null`, `docsRoot: null`, `activeSpace: "intent"`, `theme: "light"`, `spacePalette: "classic"`, `language: "en"`, `writingFont: "sans"`, `documentSort: "updated"`, `documentDensity: "full"`로 시작한다. Human root가 없으면 `언어 → 테마 + Human·AI 색상 → Human 폴더` 3단계 onboarding을 표시한다. 언어는 English, 표시상 Step 2는 Two-Tone(`charcoal`)과 Classic(`classic`)을 pre-select하고 두 설정을 독립적으로 즉시 적용·저장하며 skip할 수 있다. Step 2 skip은 Two-Tone과 Classic을 복원한다. Human 폴더는 반드시 사용자가 선택해야 한다.
...
4. 문서 목록 pane은 선택 폴더의 Markdown 문서를 전역 `documentDensity`에 따라 Full(제목, frontmatter를 제외한 본문 스니펫 최대 2줄, updated 날짜), Medium(제목, 스니펫 1줄), Simple(제목)로 표시한다. 현재 visible 문서만 별도 batch IPC로 읽고 `path + updatedMs`로 cache한다. header action은 Human에서 `Refresh → Sort → Density → Create`, 구조 변경이 제한된 AI에서 `Refresh → Sort → Density → Open File` 순서다. `RefreshCw`는 filesystem을 재스캔하고 하나의 toggle은 최신 순(`updated`, 기본)과 현재 언어의 숫자 인식 제목 순(`title`)을 전환하며, `Rows4`/`Rows3`/`Rows2`는 `Full → Medium → Simple → Full`로 순환한다. 전역 `documentSort`와 `documentDensity`를 Human·AI가 공유한다.
5. content pane 상단은 한 줄만 사용한다. 맨 왼쪽 pane icon 다음에 tab bar를 두고, 우측 actions에는 활성 문서가 있을 때 Human/AI 공통 `RefreshCw`, PDF export, transient save status, mode icon을 둔다. PDF export는 현재 렌더 Markdown을 macOS system print dialog로 보내며 Save as PDF 결과에서 앱 chrome을 제외한다. Human tab은 badge 없는 제목 한 줄이다. AI 전역 tab은 Source Card shortcut과 동일한 compact 문자 badge + `Title`, muted `.../path1/path2` 형태의 마지막 두 parent 구간을 두 줄째에 표시하며 filename을 반복하지 않는다. 두 공간 모두 가장 오른쪽 mode icon이 `Edit → View → Split(Edit | View)`로 순환하고, AI는 content 편집만 허용하며 mutation control은 제공하지 않는다. badge/shortcut은 unique canonical source root의 최초 등장 순서에서 파생하므로 같은 root의 tab은 같은 badge를 공유하고, 한 root의 마지막 문서를 닫을 때만 뒤 root가 함께 재번호화된다. 문자 label은 `{ root, path }` identity나 settings에 저장하지 않는다. tooltip·accessible name에는 문자와 전체 경로를 제공한다.
...
- Human에서 새 tab은 Edit, AI에서 Open File한 새 tab은 View로 시작하고 두 공간 모두 mode icon으로 Edit/View/Split을 순환한다.
- 현재 문서 검색은 source body 발생 순서를 기준으로 하며 Human/AI Edit/View/Split에서 동일 active result를 반영한다. 검색 query와 active result는 저장하지 않는다.
- Human tab set은 root-local이다. AI는 모든 open document의 `{ root, path }` 전역 session 하나이며 열린 파일의 본문 편집과 autosave를 허용한다. 재시작에는 reference 목록과 active reference만 저장하고 mode는 runtime state로만 유지하므로 다시 View로 열린다. 존재하지 않는 reference만 복원에서 제외한다.
...
모든 Human native 파일 작업은 `libraryRoot`를 canonicalize하고 대상 경로와 비교한다. AI Open File은 선택한 absolute Markdown file을 canonicalize한 뒤 parent root와 filename으로 분해하고 이후 read는 그 경계를 사용한다. Markdown 상대 이미지 read도 해당 문서의 root를 canonicalize하고 root containment와 regular-file·지원 MIME을 확인한 뒤 bytes만 반환한다.
...
- AI `{ root, path }` session (`docs`): OS Open File로 연 결과 문서의 본문을 편집할 수 있으며 mode는 runtime-only다. `docsRoot`는 active document source projection이다.
...
- 실제 앱에서 3단계 Human onboarding의 표시상 Step 2에 Theme과 Space Palette를 모두 제공하고 작은 창에서도 상단부터 navigation까지 스크롤 가능한지 확인한다. 동일 root의 a.md·b.md와 다른 root의 c.md Open File, same-root tab의 공유 A badge·단일 A shortcut 및 cross-root B 배정, 마지막 same-root tab close 후 shortcut 제거·동시 재번호화, shortcut row `FilePlus2`의 Open File picker, dropdown row close와 연속 close focus, 명확한 keyboard focus, AI content edit와 구조 변경 제한, Human CRUD/context menu, 다중 tab, pane 단축키, 테마 4종, Space Palette 4종, typography와 언어 재시작 복원을 확인한다.
