...
- AI는 항상 사용자가 선택한 폴더를 열며 독립적인 Browse와 Pinned mode를 제공한다. Browse는 하나의 교체 가능한 canonical 폴더와 root-local session을, Pinned는 위치 제한 없이 선택한 여러 canonical 폴더와 root별 session을 복원한다.
...
- AI Browse용 content-editable canonical `docsRoot`와 root-local `docs` session, Pinned용 ordered `{ root, label }` entries, `docsPinnedRoot`, root-local `docsPinned` sessions (내부 `docs` key 유지)
...
- Human active root picker, AI Browse 단일 root 교체, AI Pinned 사용자 label shortcut·문서 활성화
...
클린 settings는 `libraryRoot: null`, `docsRoot: null`, `docsSourceMode: "browse"`, `docsPinnedRoots: []`, `docsPinnedRoot: null`, `activeSpace: "intent"`, `theme: "light"`, `spacePalette: "classic"`, `language: "en"`, `writingFont: "sans"`, `documentSort: "updated"`, `documentDensity: "full"`로 시작한다. Human root가 없으면 `언어 → 테마 + Human·AI 색상 → Human 폴더` 3단계 onboarding을 표시한다. 언어는 English, 표시상 Step 2는 Two-Tone(`charcoal`)과 Classic(`classic`)을 pre-select하고 두 설정을 독립적으로 즉시 적용·저장하며 skip할 수 있다. Step 2 skip은 Two-Tone과 Classic을 복원한다. Human 폴더는 반드시 사용자가 선택해야 한다.
...
3. switcher 아래의 Source Card는 항상 78px, 39px 두 줄을 유지한다. AI 첫 줄 맨 앞에는 compact `Browse | Pinned`(`일반 | 고정`) selector가 있다. Browse는 shortcut 없이 Open Folder와 active canonical root row를 사용한다. Pinned는 pin 순서의 1~2 grapheme custom label shortcut과 Pin Folder, active canonical root row를 사용한다. mode, active root, space 전환 전에 열린 문서를 모두 저장하며 실패하면 기존 mode·root·tab·buffer를 유지한다.
...
- Human, AI Browse, AI Pinned tab set은 모두 root-local이다. Browse는 `docsRoot`의 `paths`와 `activePath`, Pinned는 canonical root별 `paths`와 `activePath` session을 저장한다. 두 AI mode의 session은 독립적이며 mode/root 전환 전 save barrier를 공유한다. Pinned root가 일시적으로 없으면 pin, label, session을 제거하지 않고 refresh 복구를 허용한다.
- AI root 선택은 대상 root scan 성공 후 active를 바꾸고 root-local tab과 folder navigation을 복원한다. 실패 시 기존 active root와 tab을 유지한다.
- Browse는 shortcut이나 document dropdown을 만들지 않는다. Pinned menu는 ordered canonical pins의 projection이며 label edit은 root/session identity를 유지하고 Unpin은 해당 root-local session만 제거하며 디스크 파일은 변경하지 않는다.
...
v0.2는 사용자가 편집하는 Human 원본과 Browse 또는 Pinned로 관리하는 AI 결과를 분리한다. 사용자 표시명은 Human/AI이고 내부 key와 Browse의 `docsRoot`는 유지한다.

- Human `libraryRoot` (`intent`): 인간이 직접 작성하는 canonical source-of-truth, editable
- AI Browse (`docs`): OS Open Folder로 선택한 단일 canonical `docsRoot`와 root-local tab session이다.
